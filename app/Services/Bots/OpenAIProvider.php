...
class OpenAIProvider implements BotProvider
{
    public function __construct(private Client $http, private string $key, private string $model) {}

    public function name(): string { return 'openai:'.$this->model; }

    public function reply(string $prompt, array $options = []): string
    {
        $r = $this->http->post('https://api.openai.com/v1/chat/completions', [
